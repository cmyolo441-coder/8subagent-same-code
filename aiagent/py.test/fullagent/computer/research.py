...
class Research:
    ...
    def _cached(self, key, fn):
        # Single-flight per key: 64 agents asking the same source/query
        # share ONE in-flight fetch rather than issuing a cache stampede.
        while True:
            self.control()
            with self.lock:
            ...
            if owner:
                break
            # Never wait indefinitely or occupy another executor; followers
            # share this request and stay cancellation/deadline aware.
            while not flight.wait(0.1):
                self.control()
        try:
            value = fn()
            with self.lock:
            ...
            if not isinstance(exc, (Cancelled, BudgetExceeded)):
                with self.lock:
                    self.failures[key] = (time.monotonic(), safe_text(f"Shared source request failed: {exc}", 500))
                    while len(self.failures) > 96:

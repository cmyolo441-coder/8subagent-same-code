...
class HTTPFixture:
    ...
    def __init__(self, responses):
        self.responses=list(responses);self.received=[];self.received_headers=[]
    def __enter__(self):
        fixture=self
        class Handler(BaseHTTPRequestHandler):
            ...
            def do_POST(self):
                ...
                fixture.received_headers.append(dict(self.headers))
                spec=fixture.responses.pop(0);status,ctype,body=spec[:3]
                self.send_response(status);self.send_header('Content-Type',ctype)
                self.send_header('Content-Length',str(len(body)))
                for key,value in (spec[3] if len(spec)>3 else {}).items():self.send_header(key,value)
                self.end_headers()

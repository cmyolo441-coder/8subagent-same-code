# FullAgent — Computer Mode setup and eight-lead reference

> **3.4 update:** New missions retain **8 leads + 64 children** by default. Local workers default to 8; model API traffic has a separate adaptive gate (initial 1, default cap 2 per domain). Read `API_RESILIENCE.md` for overload/recovery behavior and `HIERARCHY_MODE.md` for delegation. This guide covers provider setup and the underlying eight-lead workflow; `/computer swarm off` selects eight-lead-only mode while idle.
...
`/computer api` shows configured/learned API capacity, actual in-flight/queued work, shared cooldowns and recovery probes without making a model call. See `API_RESILIENCE.md` for temporary versus user-action failures.
...
The orchestration process does not load model weights, Docker daemons, browsers, vector-database servers or eight heavyweight copies of the main agent. It uses bounded contexts, a thread pool and existing dependencies. The default uses eight local worker slots but a separate model API gate: initially one request per domain, up to a configured default of two as capacity permits. API-waiting jobs release their worker slot. Only one host command runs at a time.
...
- `wall_minutes` includes paused time, approval waits and API recovery waits.
...
- `state.json`: atomically written version-3 checkpoint and shared task board (versions 1 and 2 remain readable).
- `continuations/`: bounded checksummed worker conversations, tool receipts and explicit uncertain-action records. Queued work has no extra model call/reservation.
...
Temporary API failures preserve committed conversations/tool receipts rather than restarting the worker. Fresh acceptance evidence on explicit resume refreshes read-only reviews, not completed implementation. An uncertain command or peer-note effect is preserved for reconciliation, never erased by automatic repair. Recovery cannot guarantee exactly-once semantics for an arbitrary external command that was interrupted after making a side effect. Inspect the logs and actual state before approving a rerun. Backups are manual recovery evidence, not a complete transaction across processes or external services.
